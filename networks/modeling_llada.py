...
from networks.configuration_llada import (
    LLaDAConfig,
    StrEnum,
    InitFnType,
    ActivationType,
...
class RotaryEmbedding(nn.Module):
    ...
    def get_rotary_embedding(self, seq_len: int, device: torch.device, position_ids=None) -> Tuple[torch.Tensor, torch.Tensor]:
        if (
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            if position_ids is not None:
                seq = position_ids                                    # (bs, len)
                freqs = einsum("b i , j -> b i j", seq, inv_freq)     # (bs, len, dim / 2)
                positions = torch.cat((freqs, freqs), dim=-1)         # (bs, len, dim)
                pos_sin, pos_cos = positions.sin()[:, None, :, :], positions.cos()[:, None, :, :]
                return pos_sin, pos_cos                               # (bs, 1, len, dim)
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, position_ids=None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            pos_sin, pos_cos = self.get_rotary_embedding(key_len, q_.device, position_ids=position_ids)
